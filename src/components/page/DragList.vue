<template>
    <section class="main">
        ...
        <div class="container">
            <!-- 提示栏 -->
            <div class="plugins-tips">Vue.Draggable:基于Sortable.js的Vue拖拽组件。访问地址:
            ...
            <div class="drag-box">
                ...
                <div class="drag-box-item">
                    ...
                    <!-- 拖拽容器v-model绑定todo数组，remove从列表中移除元素时触发。:options通过绑定options来设置配置项 -->
                    <draggable v-model="todo" @remove="removeHandle" :options="dragOptions">
                        <!-- 列表过渡 tag指定transition-group渲染为指定的元素-->
                        <transition-group tag="div" id="todo" class="item-ul">
                            <!-- 循环todo列表，渲染todo列表中的内容 -->
                ...
                <div class="drag-box-item">
                    ...
                    <!-- 拖拽容器v-model绑定doing数组，remove从列表中移除元素时触发。:options通过绑定options来设置配置项 -->
                    <draggable v-model="doing" @remove="removeHandle" :options="dragOptions">
                        <!-- 列表过渡 tag指定transition-group渲染为指定的元素-->
                        <transition-group tag="div" id="doing" class="item-ul">
                            <!-- 循环doing列表，渲染doing列表中的内容 -->
                ...
                <div class="drag-box-item">
                    ...
                    <!-- 拖拽容器v-model绑定done数组，remove从列表中移除元素时触发。:options通过绑定options来设置配置项 -->
                    <draggable v-model="done" @remove="removeHandle" :options="dragOptions">
                        <!-- 列表过渡 tag指定transition-group渲染为指定的元素-->
                        <transition-group tag="div" id="done" class="item-ul">
                            <!-- 循环done列表，渲染done列表中的内容 -->
    ...
    data() {
        return {
            dragOptions: {
                // 动画时间 单位ms
                animation: 120,
                // 当排序的容器是一个Kevin滚动的区域，拖放可以引起区域滚动
                scroll: true,
                // 分组用的，同一组的不同list可以相互拖动
                group: 'sortlist',
                // selector格式为简单css选择器的字符串，当拖动列表单元时会生成一个副本作为影子单元来模拟被拖动单元排序的情况
