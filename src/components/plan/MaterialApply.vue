        ...
        <div class="container">
            ...
            <div class="handle-box">
                ...
                <el-button type="primary" class="handle-del mr10" @click="handleAuditOne"
                    v-if="permission.includes('MaterialApply:handleAuditOne')">生产部门审核</el-button>
                <el-button type="primary" class="handle-del mr10" @click="handleAuditThree"
                    v-if="permission.includes('MaterialApply:handleAuditThree')">资材经理审核</el-button>
            ...
            <!-- row-key="id" 
                :tree-props="{ children: 'plan_apply_item_arr', hasChildren: 'hasChildren' } -->
            <el-table :data="tableData" border class="table" ref="multipleTable" header-cell-class-name="table-header"
                ...
                <el-table-column :label="$t('default.Action')" align="center">
                    <template v-slot="scope">
                        <el-button v-if="permission.includes('MaterialApply:handleAuditOne') && scope.row.status == 1"
                            type="text" @click="handleEdit(scope.$index, scope.row)">{{ $t('default.Audit') }}</el-button><!-- 审核 -->
                        <el-button
                            v-if="permission.includes('MaterialApply:handleAuditThree') && scope.row.status == 4"
                            type="text" @click="handleEdit(scope.$index, scope.row)">{{ $t('default.Audit') }}</el-button><!-- 审核 -->
                    </template>
                </el-table-column>
                ...
                <el-table-column prop="status_name" :label="$t('default.Status')">
                    ...
                    <template v-slot="scope">
                        <el-popover trigger="hover" placement="top">
                            <p>{{ scope.row.status_name }}{{ scope.row.order_status_name }}</p>
                            <div slot="reference" class="name-wrapper">
                                <el-button type="text" style="color: rgb(39, 36, 36);">{{ scope.row.status_name }}{{
                    scope.row.order_status_name }}</el-button>
                            </div>
                        </el-popover>
                    </template>
                </el-table-column>
...
export default {
    ...
    methods: {
        ...
        getData() {
            ...
            this.axios.get('/plan/MaterialApply/index', {
        ...
        saveEdit() {
            ...
            if (typeof (data_form.id) == 'undefined') {
                this.axios.post('/plan/MaterialApply/save', {
                    token: localStorage.getItem("token"),
                    params: data_form
                }).then((res) => {
                ...
                this.axios.post('/plan/MaterialApply/update', {
                    token: localStorage.getItem("token"),
                    params: data_form
                }).then((res) => {
                    console.log(res);
        ...
        savePass() {
            ...
            if (length) {
                ...
                this.axios.post('/plan/MaterialApply/auditPass', {
                    token: localStorage.getItem("token"),
        ...
        auditDetailPass() {
            ...
            this.axios.post('/plan/MaterialApply/auditDetailPass', {
                token: localStorage.getItem("token"),
                // selection: this.multipleSelection,
                params: this.form,
                // audit_type: this.audit_type,
        ...
        saveDetailFail() {
            this.axios.post('/plan/MaterialApply/auditDetailFail', {
                token: localStorage.getItem("token"),
                // selection: this.multipleSelection,
                params: this.form,
                // audit_type: this.audit_type,
        ...
        saveFail() {
            ...
            if (length) {
                ...
                this.axios.post('/plan/MaterialApply/auditFail', {
                    token: localStorage.getItem("token"),
                    selection: this.multipleSelection,
                    params: this.form,
                    audit_type: this.audit_type
        ...
        handleClick(tab, event) {
            ...
            if (tab._props.label == '生产部门审核') {
                ...
                this.query.status = 6
            } else if (tab._props.label == '资材经理驳回') {
                this.query.status = 7
