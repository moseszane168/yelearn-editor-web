    ...
    <div>
        ...
        <div class="container">
            ...
            <div class="handle-box">
                {{$t('default.Purchase_Order_Number')}}:<el-input v-model="query.pur_code"  :placeholder="$t('default.Please_enter_a_keyword')" class="handle-input mr10 w150"></el-input>
                 <!-- 推送状态 -->
                {{ $t('default.Push_Status') }}:<el-select filterable v-model="query.push" :placeholder="$t('default.Please_select')" clearable class="w120">
            ...
            <el-table :data="tableData" border class="table" ref="multipleTable" header-cell-class-name="table-header"
                ...
                <el-table-column prop="forderstatus_name" :label="$t('default.U8Status')"></el-table-column>

                <el-table-column prop="audit_push_name" label="推送状态"></el-table-column>

                <el-table-column prop="data" width="300" label="提交参数">
                    <template v-slot="scope">
                        <el-popover trigger="hover" placement="top">
                            <p>{{ scope.row.data }}</p>
                            <div slot="reference" class="name-wrapper">
                                <el-button type="text" style="color: rgb(39, 36, 36);">{{ scope.row.audit_data }}</el-button>
                            </div>
                        </el-popover>

                    </template>
                </el-table-column>
                <el-table-column prop="return_date" width="300" label="返回参数">
                    <template v-slot="scope">
                        <el-popover trigger="hover" placement="top">
                            <p>{{ scope.row.return_date }}</p>
                            <div slot="reference" class="name-wrapper">
                                <el-button type="text" style="color: rgb(39, 36, 36);">{{ scope.row.audit_return
                                    }}</el-button>
                            </div>
                        </el-popover>
...
export default {
    ...
    methods: {
        ...
        getData() {
            ...
            this.axios.get('/u8api/U8Order/indexAudit', {
                params: this.query
            }).then((res) => {
        ...
        Push() {
            ...
            if (length) {
                ...
                this.axios.post('/u8api/U8Order/pustAudit', {
                    token: localStorage.getItem("token"),
                    id_arr: id_arr,
                    params: this.form
