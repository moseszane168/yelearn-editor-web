    ...
    <div>
        ...
        <div class="container">
            ...
            <div class="handle-box">
                ...
                <el-input v-model="query.pi_code" class="handle-input mr10"></el-input>
                <!-- 部门 -->
                ...
                <!-- 领料人 -->
                {{ $t('default.Receiver') }}:<el-select v-model="query.picking_user" filterable remote reserve-keyword :placeholder="$t('default.Please_enter_a_keyword')"
                ...
                <el-button type="primary" class="handle-del mr10" @click="handleEnd"
                    ...
                    <!-- 手动完成 -->
                <el-button type="primary" class="handle-del mr10" @click="handleSelectOne">{{ $t('default.Manual_Completion') }}</el-button>
                    <!-- 手动完成 -->
                    ...
                    <template v-slot="scope">
                        <div v-if="scope.row.is_main == 1 && scope.row.status == 1">
                            <el-button type="text" @click="handleEdit(scope.$index, scope.row)"
                                v-if="permission.includes('PickingApply:handleEdit')">{{ $t('default.Edit') }}</el-button><!-- 编辑 -->
                        </div>
                    </template>
                </el-table-column>
        ...
        <el-dialog title="领料申请" :visible.sync="AddVisible" width="90%" :before-close="handleClose">
            ...
            <el-button type="primary" @click="getInventoryByMaterial">{{ $t('default.Inventory_Enquiry') }}</el-button><!-- 库存查询 -->
            <el-button type="primary" @click="downloadFile(download_file)">{{ $t('default.Download_Import_Template') }}</el-button><!-- 下载导入模板 -->
            <el-button type="primary" @click="VerifyToBuy">库存不足转物料申请</el-button>
            ...
            <el-table :data="applyDate" style="width: 100%" >
                ...
                <el-table-column prop="production_demand" label="*需求量">
                    <template slot-scope="scope">
                ...
                <el-table-column prop="mat_model" label="使用机型">
                    <template slot-scope="scope">
                ...
                <el-table-column prop="apply_num" label="采购数">
                    <template slot-scope="scope">
        ...
        <el-dialog :title="$t('default.Edit')" :visible.sync="editVisible" width="80%" :before-close="handleClose">
            <el-form ref="form" :rules="rules" :model="form" label-width="200px">
                <!-- 领料人 -->
                <el-form-item :label="$t('default.Receiver')">
                    <el-select v-model="form.picking_user" filterable remote reserve-keyword :placeholder="$t('default.Please_enter_a_keyword')"
                        :remote-method="remoteMethodUser" :loading="loading">
                        <el-option v-for="item in user_options" :key="item.id" :label="item.name" :value="item.id">
                        </el-option>
                    </el-select>
            ...
            <template>
                ...
                <el-table-column prop="desc" :label="$t('default.Desc')" >
                    ...
                    <el-table-column prop="available_num" label="可用库存"></el-table-column>
                    <el-table-column prop="transit_num" label="在途库存"></el-table-column>
...
export default {
    ...
    methods: {
        ...
        handleClick(tab, event) {
            ...
            if (tab._props.label == '待出库') {
                this.query.status = '1,2'
            } else if (tab._props.label == '已完成') {
                this.query.status = 4
