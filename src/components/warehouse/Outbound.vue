<template>
    ...
    <div>
        ...
        <div class="container">
            ...
            <div class="handle-box">
                ...
                {{ $t('default.Department') }}：<el-select v-model="query.department_id" multiple filterable remote
                    ...
                    :loading="loading" class="w200" @change="changeIndex()">
                ...
                <el-button type="primary" class="handle-del mr10" @click="handleOut"
                    v-if="permission.includes('Outbound:handleOut')">{{ $t('default.Other_Out') }}</el-button>
                <el-button type="primary" class="handle-del mr10" @click="saveAuditPass"
                    v-if="permission.includes('Outbound:handleAudit')">{{
                    $t('default.Signature') }}</el-button><!-- 签字 -->
                <el-button type="primary" class="handle-del mr10" @click="saveAuditFail"
                    v-if="permission.includes('Outbound:handleCancelAudit')">{{
            ...
            <el-table :data="tableData" border class="table" ref="multipleTable" header-cell-class-name="table-header"
                ...
                <el-table-column :label="$t('default.Action')" align="center">
                    <template v-slot="scope">
                        <div v-if="isWareSuper || scope.row.create_user == uid">
                            <el-button type="text" @click="handleEdit(scope.row.ob_code, scope.row.department_id)"
                                v-if="scope.row.status == 2 && permission.includes('Outbound:handleEdit')">{{
                    $t('default.Edit') }}</el-button><!-- 编辑 -->
                            <el-button type="text" class="red" :title="$t('default.Are_you_sure_you_want_to_delete')"
                                @click="handleDelete(scope.$index, scope.row)"
                                v-if="scope.row.status == 2 && permission.includes('Outbound:handleDelete')">{{
                ...
                <el-table-column prop="ob_code" :label="$t('default.Incoming_Batch')"><!-- 单据号 -->
                    <template v-slot="scope">
                        <el-button type="text" class="handle-del mr10" @click="handleWi(scope.row.ob_code)"> {{
        ...
        <!-- 其他出库 -->
        <el-dialog :title="$t('default.Other_Out')" :visible.sync="orderVisible" width="70%"
            ...
            <el-form :inline="true" ref="form" :rules="rules" :model="form" label-width="200px">
                ...
                <el-form-item :label="$t('default.Department')" prop="department_id">
                    <el-select v-model="form.department_id" filterable remote  @change="change()">
                        <el-option v-for="item in department_options" :key="item.value" :label="item.label"
                            :value="item.value">
                        </el-option>
                    </el-select>
            ...
            <template>
                ...
                <!-- 获取有库存仓库 -->
                <el-button type="primary" @click="wareChange">{{ $t('default.Get_Inventory_Warehouse') }}</el-button>
                ...
                <el-table :data="orderData" style="width: 100%" show-summary>
                    <!-- 物料编码 -->
                    <el-table-column prop="mat_code" :label="$t('default.Material_Code')">
                        <template slot-scope="scope">
                            <el-select v-model="scope.row.mat_code" filterable remote reserve-keyword
                                ...
                                :loading="loading" @change="wareChange()" transfer="true" :popper-append-to-body="false">
...
export default {
    ...
    data() {
        return {
            ...
            rules: {
                ...
                cdispatcherid: [
                    { required: true, message: '请输入收发类别', trigger: 'blur' },
                ],
            },
            value: [],
            list: [],
    ...
    methods: {
      ...
      changeIndex() {
        ...
        handleSave() {
            ...
            this.axios.post('/ware/Outbound/save', {
                token: localStorage.getItem("token"),
                params: this.form,
                orderData: this.orderData
    ...
    created() {
        ...
        this.query_ware_options = this.ware_options;
