    ...
    <div>
        ...
        <div class="container">
            ...
            <div class="handle-box">
                ...
                <!-- 搜索输入框 双向绑定数据，实时搜索 -->
                <!-- 单据号 -->
                {{ $t('default.Incoming_Batch') }}:<el-input v-model="query.code" :placeholder="$t('default.Please_enter_a_keyword')" class="handle-input mr10 w150"></el-input>
                <!-- 物料编码 -->
                ...
                <!-- 供应商 -->
                {{ $t('default.Supplier') }}：<el-select v-model="query.sup_code" multiple filterable remote
                    reserve-keyword :placeholder="$t('default.Please_enter_a_keyword')" :remote-method="remoteMethod"
                    :loading="loading" class="w200">
                ...
                <el-select v-model="query.department_id" multiple filterable remote
                    reserve-keyword :placeholder="$t('default.Please_enter_a_keyword')" :remote-method="remoteMethod"
                    :loading="loading" @change="getWare()" class="w200">
                    <el-option v-for="item in department_options" :key="item.value" :label="item.label"
                        :value="item.value">
                    </el-option>
                </el-select>
                <!-- 仓库 -->
                {{ $t('default.Warehouse') }}：<el-select filterable v-model="query.ware_id"
            ...
            <el-table :data="tableData" border class="table" ref="multipleTable" header-cell-class-name="table-header"
                ...
                :tree-props="{ children: 'children', hasChildren: 'hasChildren' }" show-summary
               ...
               <!-- 部门 -->
                ...
                <el-table-column prop="pur_user_name" label="采购员"></el-table-column>
                <!-- 供应商 -->
                ...
                <el-table-column prop="cdispatcherid_name" :label="$t('default.Receiving_and_Sending_Category')"
                    ...
                    <template v-slot="scope">
                        <el-popover trigger="hover" placement="top">
                            <p>{{ scope.row.cdispatcherid_name }}</p>
                            <div slot="reference" class="name-wrapper">
                                <el-button type="text" style="color: rgb(39, 36, 36);">{{ scope.row.cdispatcherid_name
                                    }}</el-button>
                            </div>
                        </el-popover>
                    </template>
