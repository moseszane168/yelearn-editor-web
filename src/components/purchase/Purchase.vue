        ...
        <div class="container">
            ...
            <div class="handle-box">
                ...
                <el-button type="primary" class="handle-del mr10" @click="handleFix"
                    v-if="permission.includes('Purchase:Fix')">{{ $t('default.Fix') }}</el-button>
                <!-- 导出明细 -->
...
<!--                    $t('default.Manual_Completion') }}</el-button>-->
            ...
            <el-table :data="tableData" border class="table" ref="multipleTable" header-cell-class-name="table-header"
                @selection-change="handleSelectionChange" :row-class-name="tableRowClassName" max-height="550px">
                <!-- 操作 -->
                <el-table-column :label="$t('default.Action')" align="center" width="120">
                    <template v-slot="scope">

                        <el-button type="text" @click="handlePurUpdate(scope.row.pur_code, scope.row.sup_code)"
...
<!--                            v-if="permission.includes('Purchase:handleReturn')">{{ $t('default.Returns') }}</el-button>-->
                ...
                <el-table-column prop="total_price" :label="$t('default.Total_Price')" v-if=cost_show>
                    <template v-slot="scope">
                        <el-button style="color: rgb(39, 36, 36);" type="text"> {{
                    scope.row.total_price }}</el-button>
                    </template>
                </el-table-column>
                <!-- 退款 -->
                <el-table-column v-if=cost_show prop="return_price" :label="$t('default.Refund')">
                    <template v-slot="scope">
                        <el-button style="color: rgb(39, 36, 36);" type="text"> {{
                    scope.row.return_price }}</el-button>
                    </template>
                </el-table-column>
              ...
              <!-- 仓库签字 -->
              <el-table-column prop="estimated_delivery_time" label="仓库签字" width="105">

                <template v-slot="scope">
<!--                  <el-tag v-if="scope.row.wi_status == '1'" type="success">已签字</el-tag>
                  <el-tag v-if="scope.row.wi_status != '1'" type="warning">待签字</el-tag>-->
                  <el-tag type="warning">{{scope.row.sign_status}}</el-tag>
                </template>
              </el-table-column>
                <!-- 供应商 -->
...
export default {
    ...
    methods: {
        ...
        handlePurUpdate(pur_code, sup_code) {

            this.orderVisible = true;
            this.$nextTick(() => {
                this.$refs.purchaseUpdate.dataInitialization(pur_code, sup_code);
            })
        },
        reload() {
        ...
        handleExport() {
            ...
            if (length > 0) {
              for (let i = 0; i < length; i++) {
                console.log(this.multipleSelection[i].pur_code)
                this.query.pur_codes.push(this.multipleSelection[i].pur_code);
              }
            }
